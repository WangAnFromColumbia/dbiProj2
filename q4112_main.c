...
int main(int argc, char* argv[]) {
  ...
  FILE* fileptr = fopen("./q4112_wangshuo_main.csv", "a");
  ...
  // run using various number of threads and 5 repeats
  int threads_arr[5] = {1, 2, 4, 8, 16};
  int threads_index, i;
  // number of threads
  for (threads_index = 0; threads_index < 5; ++threads_index) {
    threads = threads_arr[threads_index];
    // number of repeats
    for (i = 1; i <= 5; ++i) {
      ...
      fprintf(fileptr, "%ld,%.1f,%u,%ld,%.1f,%u,%ld,%ld,%.1f,%d,%d,%llu\n", inner_tuples, inner_selectivity,
              inner_val_max, outer_tuples, outer_selectivity, outer_val_max, groups, hh_groups, hh_probability, threads,
              i, run_ns);
    }
  }
  free(inner_keys);
  ...
  free(outer_vals);
  fclose(fileptr);
  total_ns = real_time() - total_ns;
  fprintf(stderr, "Total Execution time:  %12s ns\n", add_commas(total_ns));
  return EXIT_SUCCESS;
}
